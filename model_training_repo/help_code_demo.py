...
def loadCSV(csvf):
    ...
    with open(csvf) as csvfile:
        ...
        for i, row in enumerate(csvreader):
            ...
            # if filename in dictLabels.keys():
...
class IEGM_DataSET():
    ...
    def __getitem__(self, idx):
        ...
        # IEGM_seg = txt_to_numpy(text_path, self.size).reshape(1, self.size, 1)  # for 2d
        IEGM_seg = txt_to_numpy(text_path, self.size).reshape(1, self.size)     # for 1d
        label = int(self.names_list[idx].split(' ')[1])
        sample = {'IEGM_seg': IEGM_seg, 'label': label}

        # if self.transform:
        #     data = self.transform(IEGM_seg)
...
def pytorch2onnx(net_path, net_name, size):
    ...
    dummy_input = torch.randn(1, 1, size)
