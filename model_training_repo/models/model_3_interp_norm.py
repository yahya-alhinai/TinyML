...
class IEGMNet(nn.Module):
    def __init__(self):
        ...
        self.interp = nn.Sequential(
            Interpolate(scale_factor=0.4, mode="area") # length=500(x) =350
        )
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=3, kernel_size=3, stride=2, padding=0),
            nn.ReLU(True),
            nn.BatchNorm1d(3, affine=True, track_running_stats=True, eps=1e-5, momentum=0.1),
        )

        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=5, kernel_size=3, stride=2, padding=0),
            nn.ReLU(True),
            nn.BatchNorm1d(5, affine=True, track_running_stats=True, eps=1e-5, momentum=0.1),
        )

        self.conv3 = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=10, kernel_size=3, stride=2, padding=0),
            nn.ReLU(True),
        ...
        self.fc1 = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(in_features=260, out_features=10)
        )
    ...
    def forward(self, input):
        ...
        interp_output = self.interp(norm_output)
        # print(interp_output.shape)
        conv1_output = self.conv1(interp_output)
