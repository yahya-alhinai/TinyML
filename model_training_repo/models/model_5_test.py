...
class IEGMNet(nn.Module):
    def __init__(self):
        ...
        self.pool = nn.Sequential(
            nn.MaxPool1d(kernel_size=5, stride=4, padding=0) # size=416
        )
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=5, kernel_size=6, stride=3, padding=0),
            nn.ReLU(True),
            nn.BatchNorm1d(5, affine=True, track_running_stats=True, eps=1e-5, momentum=0.1),
        )

        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=5, out_channels=5, kernel_size=6, stride=3, padding=0),
            nn.ReLU(True),
            nn.BatchNorm1d(5, affine=True, track_running_stats=True, eps=1e-5, momentum=0.1),
        )
...
####################
    ...
    def forward(self, input):
        pool_output = self.pool(input)
        norm_output = self.norm(pool_output)
        ...
        conv3_output = conv3_output.view(-1,50)
